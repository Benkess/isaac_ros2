...
* **Apptainer image**: `/containers/ros2_humble.sif` & `/containers/isaac-sim.sif`
...
> **ros\_overlay.img** is your writable layer for installing OS or ROS packages without modifying the base container.
...
```
> Note: use '--fakeroot' to modify the overlay (apt)

```bash
source /opt/ros/humble/setup.bash
cd /humble_ws
-H apt-get update && apt-get install -y ros-humble-joint-state-publisher
```
